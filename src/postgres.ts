...
export type PostgresClientOptions = Pick<PoolConfig, 'user' | 'password' | 'host' | 'port' | 'database'>;
...
export class PostgresClient {
  ...
  constructor(options: PostgresClientOptions) {
    this.connectionString = new URL(`postgres://${options.user}:${options.password}@${options.host}:${options.port}/${options.database}`).href;
